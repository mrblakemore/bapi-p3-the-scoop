...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    const userArticleIds = database.users[savedComment.username].articleIds;
    userArticleIds.splice(userArticleIds.indexOf(id), 1);
    const userAutherIds = database.users[savedComment.username].commentIds;
    userAutherIds.splice(userAutherIds.indexOf(id), 1);
    //last one
...
function editUpvote(url, request) {
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const username = request.body && request.body.username;
...
function editDownvote(url, request) {
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const username = request.body && request.body.username;
  let savedComment = database.comments[id];
  const response = {};
